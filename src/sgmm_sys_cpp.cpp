...
// [[Rcpp::export]]
List sgmm_sys_cpp(const arma::mat& x, 
                 const arma::colvec& y, 
                 const arma::mat& z, 
  ...
  double m_i;
  arma::mat z_i = trans(z.rows(0, n_eq-1));
  arma::mat G_i = z_i * x.rows(0, n_eq-1);
  arma::vec H_i = - z_i * y.rows(0, n_eq-1);
  ...
  arma::mat so_mul;
  arma::mat so_inv;
  // Something wront before this line
  ...
  for (int obs = 1; obs < (n1+1); obs++){
    ...
    H_i = - z_i * y.rows( ((obs-1)*n_eq), (obs*n_eq-1));
