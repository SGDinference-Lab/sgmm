...
tmp1 = solve(t(Phi_start3)%*%w_start3%*%Phi_start3)
tmp2 = t(Phi_start3)%*%w_start3

snorm_result = matrix(NA, nrow=n0, ncol=1)
for (i in 1:n0){
  tmp_i = (as.matrix(z0_s3[i,]))%*%t(as.matrix(x0_s3[i,]))
  tmp = tmp1%*%tmp2%*%tmp_i 
  snorm = norm(tmp, type="2")
  snorm_result[i,]=snorm/ncol(x0_s3) 
}
Psi0 = quantile(snorm_result,0.5)
gamma0 = 1/Psi0

# S2SLS
out3 = sgmm(x=x1_s3, y=y1_s3, z=z1_s3, gamma_0=gamma0, alpha=0.501, bt_start = bt_start3,
           inference = "rs", weight= "2sls_so", n0 = n0,
           Phi_start = Phi_start3, w_start = w_start3, n_perm = 5)
